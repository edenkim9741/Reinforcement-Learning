...
class SelfPlayEnvWrapper(gym.Env):
    """
    Self-play environment where the agent plays against itself.
    """
    ...
    def _get_obs_and_info(self):
        ...
        info_dict = {
            ...
            "truncation": truncation,
            "action_mask": observation["action_mask"],
            "current_player": self.aec_env.agent_selection
        }
    ...
    def step(self, action: int):
        ...
        if termination or truncation:
            rewards = self.aec_env.rewards
            # Return reward from perspective of player who just moved
            prev_player = "player_1" if self.aec_env.agent_selection == "player_0" else "player_0"
            final_reward = rewards.get(prev_player, 0.0)
            obs, info_dict = self._get_obs_and_info()
            return obs, final_reward, True, False, info_dict
        
        obs, info_dict = self._get_obs_and_info()
        return obs, 0.0, False, False, info_dict
...
# =============================================================================
# Subprocess-based Vectorized Environment for Stockfish (Parallel)
# =============================================================================
def _worker_process(remote, parent_remote, env_fn_wrapper):
...
class SubprocVecEnv:
    ...
    Each environment runs in its own process with its own Stockfish engine.
    """
    ...
    def __init__(self, env_fns):
        ...
        for work_remote, remote, env_fn in zip(self.work_remotes, self.remotes, env_fns):
            process = Process(target=_worker_process, args=(work_remote, remote, env_fn))
            process.daemon = True
            process.start()
...
class PPOTrainer:
    ...
    def __init__(self, config: Config, device: torch.device, use_parallel: bool = True, use_wandb: bool = False):
        ...
        # Setup logging
        run_name = f"ppo_chess_{int(time.time())}"
        if HAS_TENSORBOARD:
    ...
    def train(self):
        ...
        for iteration in range(1, self.num_iterations + 1):
            ...
            if self.writer is not None:
                if total_games > 0:
                    win_rate = self.win_count / total_games
                    self.writer.add_scalar("charts/win_rate", win_rate, self.global_step)
                    self.writer.add_scalar("charts/total_games", total_games, self.global_step)
                ...
                self.writer.add_scalar("losses/entropy", entropy_loss, self.global_step)
                self.writer.add_scalar("losses/clipfrac", clipfrac, self.global_step)
            ...
            if iteration % self.config.eval_interval == 0:
                ...
                if self.writer is not None:
                    self.writer.add_scalar("eval/win_rate", eval_win_rate, self.global_step)
                if self.use_wandb:
                    wandb.log({
...
# =============================================================================
# Pretraining with Stockfish
# =============================================================================
def pretrain_with_stockfish(config: Config, device: torch.device):
    """
    Pretrain the agent by imitating Stockfish moves.
    This provides a strong initialization before PPO training.
    Uses batch training for efficiency.
    """
    print("=" * 60)
    print("Pretraining with Stockfish imitation learning")
    print("=" * 60)
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=config.pretrain_episodes, eta_min=1e-5
    )
    criterion = nn.CrossEntropyLoss()
    
    try:
        engine = chess.engine.SimpleEngine.popen_uci(config.stockfish_path)
        engine.configure({"Skill Level": config.stockfish_skill})
    except FileNotFoundError:
        print(f"Stockfish not found at {config.stockfish_path}")
        return None
    ...
    for episode in range(config.pretrain_episodes):
        env.reset()
        ...
        for agent_name in env.agent_iter():
            ...
            try:
                result = engine.play(
                    board, 
                    chess.engine.Limit(time=config.stockfish_time_limit)
                )
            ...
            # Verify action is valid
            if observation["action_mask"][target_action] != 1:
                action = np.random.choice(np.where(observation["action_mask"])[0])
                env.step(action)
                continue
            
            # Add to batch
            batch_obs.append(observation["observation"].astype(np.float32))
            batch_masks.append(observation["action_mask"].astype(np.float32))
            batch_targets.append(target_action)
            
            # Train when batch is full
            if len(batch_obs) >= batch_size:
                obs_tensor = torch.from_numpy(np.array(batch_obs)).to(device)
                mask_tensor = torch.from_numpy(np.array(batch_masks)).to(device)
                target_tensor = torch.tensor(batch_targets, dtype=torch.long).to(device)
        ...
        if (episode + 1) % 50 == 0:
            ...
            print(f"Episode {episode + 1}/{config.pretrain_episodes} | "
                  f"Avg Loss: {avg_loss:.4f} | Samples: {num_samples} | "
    ...
    print(f"Pretrained model saved to {pretrain_path}")
    print(f"Total samples trained: {num_samples}")
