...
import os
import random  # [NEW] 랜덤 색상 선택을 위해 추가
import numpy as np
import torch
...
class ChessSelfPlayEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, opponent_depth=1):
        ...
        # 초기화 시 임시로 에이전트 ID 설정 (reset에서 매번 바뀜)
        self.aec_env.reset()
        self.agents = self.aec_env.agents
        self.agent_id = "player_0" 
        self.opponent_id = "player_1"

        # Observation / Action Space는 player_0 기준 (대칭적이므로 동일)
    ...
    def _opponent_step(self):
        ...
        try:
            # [IMPORTANT] 상대방이 흑(player_1)이면 mirror=True, 백(player_0)이면 mirror=False
            is_opponent_black = (self.opponent_id == "player_1")
            action_opp = encode_move(best_move, should_mirror=is_opponent_black)
        except:
            curr = self.aec_env.agent_selection
            mask = self.aec_env.observe(curr)["action_mask"]
    ...
    def reset(self, seed=None, options=None):
        ...
        if random.random() < 0.5:
            self.agent_id = "player_0"  # Agent = White
            self.opponent_id = "player_1"
        else:
            self.agent_id = "player_1"  # Agent = Black
            self.opponent_id = "player_0"

        # 내 차례가 올 때까지 상대방(Minimax) 진행
        # 내가 흑이라면, 백(상대)이 먼저 둠.
        # 내가 백이라면, while문 바로 통과.
...
# ... (RecordVideoHelper, ActorCritic, Config 등 나머지 코드는 동일) ...
# 편의를 위해 전체 코드를 아래에 이어 붙입니다.
...
@dataclass
class PPOConfig:
    exp_name: str = "ppo_chess_mixed_color"
    seed: int = 1
...
def train(config: PPOConfig):
    ...
    if config.logging:
        run = wandb.init(
            ...
            name=time.strftime("%Y-%m-%d_%H-%M-%S") + "_ppo_chess_mixed_color",
    ...
    for update in bar:
        ...
        if update % 100 == 0:
            record_chess_video(agent, device, "videos/chess_mixed", update, max_steps=300)
